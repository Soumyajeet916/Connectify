...
const StoryViewer = ({viewStory,setViewStory}) => {
  ...
  const renderContent =()=>{
    switch(viewStory.media_type){
      case 'image':
        return <img src={viewStory.media_url} alt="story" className='max-h-[90vh] max-w-[90vw] object-contain' />;
      case 'video':
        return <video src={viewStory.media_url} className='max-h-[90vh] max-w-[90vw] object-contain' controls autoPlay />;
      case 'text':
        return <div className='max-h-[90vh] max-w-[90vw] flex items-center justify-center p-8' >
          <p className='text-white text-2xl sm:text-4xl font-medium text-center' style={{color: viewStory.text_color}}>
            {viewStory.content}
          </p>
        </div>;
      default:
