    ...
    <div className='min-h-screen relative bg-slate-100'>
      <div className='max-w-7xl mx-auto p-6'>
        ...
        <div className='mb-8'>
          ...
          <p className='text-slate-600'>Stay connected. Stay in sync</p>
        </div>
        {/* Connected User */}
        <div className='flex flex-col gap-3'>
          {dummyConnectionsData.map((user) => (
            <div key={user._id} className='max-w-xl flex flex-wrap gap-5 p-6 bg-white shadow rounded-md'>
              <img src={user.profile_picture} alt="" onClick={()=> navigate( `/profile/${user._id}` )}className='rounded-full size-12 mx-auto' />
